...
class NGOProfilePage extends StatefulWidget {
  ...
  const NGOProfilePage({super.key, this.requestedUid});
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUid == null) {
      return const Scaffold(
        ...
        body: Center(
          child: Text(
            'Неуспешно зареждане на профила.',
    ...
    return StreamBuilder<NGO?>(
      ...
      builder: (context, snapshot) {
        ...
        final bool isOwner = userObj is NGO && userObj.id == ngo.id;

        return Scaffold(
          backgroundColor: backgroundGrey,
          appBar: widget.requestedUid != null
            ? AppBar(
                backgroundColor: backgroundGrey,
                elevation: 0,
                leading: IconButton(
                ...
                title: Text(ngo.name, style: const TextStyle(color: Colors.black, fontSize: 18)),
                centerTitle: true,
              )
            : null,
